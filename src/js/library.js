...
const fixedHeroUrl = new URL('../img/library-hero-fixed.svg', import.meta.url).href;
...
const refs = {
  ...
  filterButton: document.querySelector('.js-library-filter-button'),
  filterMenu: document.querySelector('.js-library-filter-menu'),
...
function renderGenreFilter() {
  ...
  refs.filterButton.textContent = selectedGenre === 'all' ? 'All genres' : selectedGenre;
  ...
  if (refs.filter) refs.filter.hidden = allMovies.length === 0;

  refs.filterMenu.innerHTML = [
    '<li><button type="button" data-genre="all">All genres</button></li>',
    ...genres.map(
      genre =>
        `<li><button type="button" data-genre="${escapeHtml(genre)}">${escapeHtml(genre)}</button></li>`
    ),
  ].join('');
}

function setFilterMenuOpen(isOpen) {
  if (!refs.filterMenu || !refs.filterButton || refs.filterButton.disabled) return;
      ...
      <button class="movie-card__button" type="button" aria-label="Open ${escapeHtml(title)} details">
        ...
        <span class="movie-card__info">
          ...
          <span class="movie-card__footer">
            <span class="movie-card__meta">${escapeHtml(getGenreText(movie))} | ${getYear(movie)}</span>
            <span
...
function renderMovieList() {
  ...
  const isEmpty = allMovies.length === 0;
  refs.empty.hidden = !isEmpty;
  refs.list.hidden = isEmpty;
  refs.loadMore.hidden = isEmpty || visibleCount >= filteredMovies.length;
}
...
function bindEvents() {
  ...
  refs.filterButton?.addEventListener('click', () => {
    ...
    setFilterMenuOpen(refs.filterMenu.hidden);
  });
  ...
  document.addEventListener('click', event => {
    if (!refs.filterMenu || !refs.filterButton) return;
  ...
  window.addEventListener('storage', event => {
    if (event.key === LIBRARY_STORAGE_KEY) refreshLibrary();
  });
}
